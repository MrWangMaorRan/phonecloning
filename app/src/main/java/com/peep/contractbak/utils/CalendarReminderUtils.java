...
public class CalendarReminderUtils {
     ...
     * 添加日历事件
     */
    public static void addCalendarEvent(Context context, CalendarBean calendarBean) {
        if (context == null) {
            return;
        }
...
//        mCalendar.setTimeInMillis(reminderTime);//设置开始时间
        long start = Long.parseLong(calendarBean.getDtstart());
//        mCalendar.setTimeInMillis(start + 10 * 60 * 1000);//设置终止时间，开始时间加10分钟
        long end = Long.parseLong(calendarBean.getDtend());
